...
uv run python ~/.claude/scripts/agent_db.py create-flag \
  ...
  --impact_level "[level]" \
  --related_files "[file1.sql,file2.py,config.json]" \
...
- Example: `--related_files "models/user.py,api/endpoints.py,config/database.json"`
...
- `breaking_change`: Existing spatial queries will break
- `new_feature`: New spatial capability available
...
# You need to update spatial data processing
...
uv run python ~/.claude/scripts/agent_db.py create-flag \
  ...
  --change_description "Spatial query API changed for ML integration" \
  --action_required "Update API endpoints to use new spatial function signatures for feature extraction" \
  --impact_level "high" \
  --related_files "functions/spatial_ml.sql,indexes/ml_spatial_idx.sql" \
  --chain_origin_id "100"
```
