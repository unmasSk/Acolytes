...
class TwilioMessagingService {
    ...
    async checkRateLimit(recipient) {
        ...
        if (now - data.windowStart > window) {
            // Reset window
            this.rateLimiter.set(key, {count: 1, windowStart: now});
        } else if (data.count >= 10) { // 10 messages per minute
    ...
    async handleTwilioError(error, originalParams) {
        ...
        if (retryableErrors.includes(error.code) && originalParams.retryCount < this.maxRetries) {
            // Exponential backoff
...
class TwilioWebhookHandler {
    ...
    async processMessageEvent(eventData) {
        const {MessageStatus, MessageSid, ErrorCode, ErrorMessage, To, From} = eventData;
...
class SendGridEmailService {
    ...
    async manageTemplate(action, templateData) {
        const request = {
        ...
        switch (action) {
            ...
            case 'update':
                request.url += `/${templateData.templateId}`;
        ...
        try {
            ...
            throw new Error(`Template ${action} failed: ${error.message}`);
        }
    }
    ...
    async manageSuppression(action, emails, groupId = null) {
        const request = {
            headers: {
                'Authorization': `Bearer ${this.apiKey}`,
        ...
        switch (action) {
            case 'add_global':
                ...
                request.body = {recipient_emails: emails};
                break;
        ...
        try {
            ...
            throw new Error(`Suppression ${action} failed: ${error.message}`);
        }
    }
    ...
    async optimizeDeliverability() {
        // Check reputation
        const reputation = await this.checkReputationMetrics();
        
        // Analyze bounce rates
        const bounceAnalysis = await this.analyzeBounces();
        
        // Review suppression lists
        const suppressionAnalysis = await this.analyzeSuppressions();
        
        return {
            reputation,
            bounceAnalysis,
            suppressionAnalysis,
            recommendations: this.generateDeliverabilityRecommendations({
                reputation,
                bounceAnalysis,
                suppressionAnalysis
            })
    ...
    generateDeliverabilityRecommendations(analysis) {
        const recommendations = [];
        
        if (analysis.bounceAnalysis.hardBounceRate > 5) {
            recommendations.push({
                priority: 'high',
                issue: 'High hard bounce rate',
                action: 'Clean email lists and implement double opt-in'
            });
        }
        
        if (analysis.reputation.score < 80) {
            recommendations.push({
                priority: 'medium',
                issue: 'Low sender reputation',
                action: 'Reduce sending frequency and improve engagement'
            });
        }
...
class FirebasePushService {
    ...
    // Advanced analytics and token management
    async analyzeDeliveryMetrics(timeRange) {
        // This would integrate with FCM analytics APIs when available
        // For now, implement custom tracking
        const metrics = await this.getCustomMetrics(timeRange);
        ...
        return {
            ...
            platformBreakdown: metrics.platformStats,
            topicPerformance: metrics.topicStats
        };
    }
...
class FCMTokenManager {
    ...
    async storeToken(userId, token, platform, appVersion) {
        ...
        await this.storage.hset(`fcm_tokens:${userId}`, token, JSON.stringify(tokenData));
    ...
    async getActiveTokens(userId) {
        ...
        for (const [token, dataStr] of Object.entries(tokenMap)) {
            ...
            if (data.isActive && Date.now() - data.lastUpdated < this.tokenRefreshInterval) {
...
class RealTimeCommunicationService {
    ...
    async queueOfflineNotifications(userIds, event, data) {
        ...
        // Add to message queue (Redis, RabbitMQ, SQS, etc.)
        await this.messageQueue.addBatch('offline_notifications', notifications);
    }
    ...
    async execute(operation, fallback = null) {
        ...
        try {
            ...
            if (this.state === 'HALF_OPEN') {
                this.successCount++;
                if (this.successCount >= 3) { // Require 3 successful calls
                    this.reset();
                }
            }
...
class CommunicationMetrics {
    ...
    recordWebhookProcessing(provider, eventType, duration) {
        this.webhookProcessingTime.observe({provider, event_type: eventType}, duration);
    }
    ...
    async getDeliveryRateForWindow(channel, window) {
        ...
        const sent = await this.queryMetrics(`messages_sent{channel="${channel}"}`, window);
        const delivered = await this.queryMetrics(`messages_delivered{channel="${channel}"}`, window);
...
class CommunicationSecurity {
    ...
    detectAndMaskPII(content) {
        ...
        for (const [type, pattern] of Object.entries(patterns)) {
            ...
            if (matches) {
                ...
                maskedContent = maskedContent.replace(pattern, `[${type.toUpperCase()}_MASKED]`);
            }
        }
    ...
    validateWebhookSignature(provider, signature, payload, timestamp) {
        ...
        switch (provider) {
            ...
            case 'sendgrid':
                return this.validateSendGridSignature(signature, payload, timestamp, secret);
            case 'stripe': // For payment webhooks
                return this.validateStripeSignature(signature, payload, timestamp, secret);
            default:
    ...
    validateSendGridSignature(signature, payload, timestamp, secret) {
        ...
        if (Math.abs(now - requestTime) > 600) { // 10 minutes tolerance
...
class CommunicationDiagnostics {
    ...
    generateRecommendations(diagnosticResults) {
        ...
        for (const [service, status] of Object.entries(diagnosticResults.services)) {
...
if (results.services.email.status === 'unhealthy') {
    await diagnostics.executeEmergencyProcedure('switch_to_backup', {channel: 'email'});
}
```

Always maintain the highest standards of reliability, security, and performance while leveraging modern communication technologies to build messaging systems that scale with application needs and provide exceptional user experiences across all channels.