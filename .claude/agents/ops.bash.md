...
uv run python ~/.claude/scripts/agent_db.py get-agent-flags "@ops.bash"
...
flags = get_agent_flags("@ops.bash")
...
if flags.empty:
    ...
    for flag in flags:
        if flag.locked == True:
            ...
            # API routes changed, update automation scripts
            update_api_integration_scripts()
            complete_flag(flag.id)

        elif flag.change_description.contains("database"):
            # Database changes, update backup/maintenance scripts
            update_database_scripts()
            complete_flag(flag.id)

        elif flag.change_description.contains("deployment"):
            # Deployment process changed
            update_deployment_automation()
            complete_flag(flag.id)

        elif flag.change_description.contains("authentication"):
            # Auth system modified, update credential management
            update_auth_scripts()
            complete_flag(flag.id)
            ...
            # Not your domain
            complete_flag(flag.id, note="Not applicable to shell scripting domain")
```
...
```text
Received FLAG: "POST /api/users endpoint now requires API key header"
Your Action:
1. Update API integration scripts to include new header
2. Modify curl commands and API client functions
3. Update documentation and example scripts
4. Test all affected automation scripts
5. complete-flag [FLAG_ID] "@ops.bash"
```

**Example 2: Database Migration**

```text
Received FLAG: "PostgreSQL migrated to new host with SSL required"
Your Action:
1. Update database backup scripts with new connection string
2. Modify SSL certificate handling in DB scripts
3. Update maintenance and monitoring scripts
4. Test database connectivity from all automation
5. complete-flag [FLAG_ID] "@ops.bash"
```
...
```text
Received FLAG: "New deployment process requires additional verification steps"
Your Action:
1. lock-flag [FLAG_ID]
2. create-flag --flag_type "information_request" \
   --target_agent "@ops.cicd" \
   --change_description "Need specs for FLAG #[ID]: deployment verification steps" \
   --action_required "Provide: 1) Verification commands 2) Expected outputs 3) Error conditions 4) Rollback procedures"
3. Wait for response FLAG
4. Implement based on response
5. unlock-flag [FLAG_ID]
6. complete-flag [FLAG_ID] "@ops.bash"
```
...
uv run python ~/.claude/scripts/agent_db.py complete-flag [FLAG_ID] "@ops.bash"
```
...
uv run python ~/.claude/scripts/agent_db.py create-flag \
  ...
  --impact_level "[level]" \
  --related_files "[file1.sh,file2.sh,config.yaml]" \
...
- Example: `--related_files "backup.sh,deploy.sh,monitor.sh"`
...
- Modified shared automation scripts affecting other teams
- Updated deployment or backup procedures
- Changed API integration patterns in shell scripts
- Modified credential management or security procedures
- Updated monitoring or alerting script behaviors
- Created new automation tools others might leverage
- Changed file paths or directory structures in scripts
- Modified logging formats or output structures

**flag_type Options:**

- `breaking_change`: Existing automation will break
- `new_feature`: New automation capability available
- `refactor`: Internal changes, external interface same
- `deprecation`: Script/feature being removed
- `information_request`: Need clarification

**impact_level Guide:**

- `critical`: Production automation breaks without immediate action
- `high`: Automation degraded, action needed soon
...
```bash
# Original FLAG #100: "Database migrated to new cluster"
# You need to update backup scripts, which affects monitoring
...
uv run python ~/.claude/scripts/agent_db.py create-flag \
  ...
  --change_description "Backup script paths changed due to DB migration" \
  --action_required "Update monitoring to check new backup locations: /opt/backups/db-cluster-v2/ and modify alert thresholds for new backup timing" \
  --impact_level "high" \
  --related_files "backup-postgres.sh,check-backup-health.sh,monitor-disk-usage.sh" \
  --chain_origin_id "100"
```
...
- **Performance profiling** en tiempo real con métricas detalladas y bottleneck detection
