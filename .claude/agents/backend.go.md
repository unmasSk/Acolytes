...
uv run python ~/.claude/scripts/agent_db.py create-flag \
  ...
  --impact_level "[level]" \
  --related_files "[file1.go,file2.js,config.json]" \
...
- Example: `--related_files "models/user.go,api/handlers.go,config/database.json"`
...
uv run python ~/.claude/scripts/agent_db.py create-flag \
  ...
  --impact_level "high" \
  --related_files "models/predictor.go,models/classifier.go,api/endpoints.go" \
  --chain_origin_id "100"
```
